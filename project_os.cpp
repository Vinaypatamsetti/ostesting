...
void rr(int num,int rem_time[10],int Cur_t,int arri_Time[10], int bsT[10]);

main() 
{
	int Proc_num,j,num_proc,C_Time,RemProc,indicator,time_quan,wait,tat,arri_Time[10],bsT[10],rem_time[10],x=1;
	indicator = 0;
	wait = 0;
	tat = 0;
	...
	for(Proc_num=0;RemProc!=0;) 
	{
		...
		if(rem_time[Proc_num]==0 && indicator==1)			
		{ printf("%d",Proc_num);
			RemProc--;				
	...
	rr(num_proc,rem_time,C_Time,arri_Time,bsT);
